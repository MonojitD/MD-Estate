    ...
    <div className='border-2 border-red-600 bg-[#00000020] fixed top-0 left-0 w-full h-full flex justify-center items-center'>
        <div className='w-[65%] max-w-md bg-white rounded-md p-3 flex flex-col items-center'>
            <div className='w-full text-center'>
                <h3 className='text-lg font-semibold text-purple-500' style={{color: modalData.color}}>{modalData.title}</h3>
            ...
            <div className='w-full flex justify-center gap-2 mt-4'>
                ...
                <button className='w-[40%] py-2 rounded-md text-sm font-medium text-slate-100 hover:-translate-y-0.5 bg-purple-500' style={{backgroundColor: modalData.color}} onClick={modalData.action}>{modalData.type}</button>
            </div>
        </div>
    </div>
  )
